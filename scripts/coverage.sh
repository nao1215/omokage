...
# real omokage binary the way an end user does; the atago E2E specs do. Go 1.20+
...
# real run covers" and get one honest number that octocov's threshold reflects.
#
# This is intentionally a separate, heavier target: `make test` and
# `make test-e2e-atago` stay fast and unchanged. Everything lands under
# .coverage/ (gitignored) except the final coverage.out / coverage.html, the
# same artifacts `make test` already produces, so octocov and local tooling
# need no changes.
set -eu
